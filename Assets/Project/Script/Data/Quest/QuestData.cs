...
public class QuestData : BaseData_ForUi
{
    ...
    public int[] prerequisiteQuestIDs; // 선행 퀘스트 ID 목록

    [Header("Progress (진행 및 완료)")]
    public BaseQuestCondition[] completionConditions;   // 완료 조건 (몬스터 처치, 아이템 수집 등)
    public BaseQuestCondition[] additionalConditions;   // 추가보상 조건
    public BaseQuestCondition[] failureConditions;    // 실패 조건 (특정 NPC 사망 등)

    [Header("Rewards (보상)")]
    public BaseQuestReward[] baseRewards;           // 확정 보상
    public BaseQuestReward[] additionalRewards;     // 추가조건 만족시 보상
    public BaseQuestReward[] optionalRewards; // 선택 보상
