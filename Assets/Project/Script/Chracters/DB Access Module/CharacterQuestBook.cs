...
namespace Farm.Character
{
    class CharacterQuestBook : BaseDatabaseAccess<QuestStaticManager, QuestData>
    {
        ...
        {
            base.Dispose();
            EventBus<GameMinutePassedEvent>.Unsubscribe(QuestTimeOutCount);
        }
        protected override void OnInitialized()
        {
            base.OnInitialized();
            //TODO: 필요하다면 여기서 세이브 데이터를 로드하여 _activeQuests와 _questClearCounts를 복구
            timeManager = CoreFacade.GetManager<TimeManager>();

            EventBus<GameMinutePassedEvent>.Subscribe(QuestTimeOutCount);
        }

        private void QuestTimeOutCount(GameMinutePassedEvent e)
        {
            QuestTimeOutCount(e.DeltaMinutes);
        }
        ...
        public bool TryCompleteQuest(int questID)
        {
            ...
            // 내부적으로 조건 달성했는지 체크
            BaseCharacter character = Host.GetComponent<BaseCharacter>();
            questContainer.EvaluateStatus(character);
